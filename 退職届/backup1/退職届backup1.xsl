			...
			<xsl:for-each select="Item">
				<fo:page-sequence master-reference="A4" font-family="IPA Gothic,IPAGothic" font-size="9">
					<fo:flow flow-name="xsl-region-body">
									...
									<fo:table-cell>
										...
										<fo:block line-height="1.0" margin-top="6mm">
											...
											<xsl:choose>
												<xsl:when test="(Sales.U_6B60BE32C518D198E59655CF81793D = '寮')">
													<fo:inline padding-left="10mm">
														<xsl:value-of select="Sales.U_73081E7CBF00C0993EA66ABF2DB9F4"/>
														<xsl:value-of select="Sales.U_1BAFD646C76F8079A0FCABABAA55C5"/>
													</fo:inline>
												</xsl:when>
												<xsl:otherwise/>
											</xsl:choose>
										...
										<!--<fo:block line-height="1.0" margin-top="6mm">
											...
											<fo:inline padding-left="6mm">
												<xsl:value-of select="concat(Sales.P_Candidate/Candidate/Person.U_6E906CC3D68B8B4F24A2BA8C5398E7, Sales.P_Candidate/Candidate/Person.P_Street)"/>
											</fo:inline>
										</fo:block>
										<fo:block line-height="0.5">
											<fo:inline padding-left="12mm">
												<fo:leader leader-pattern="rule" rule-thickness="0.3pt" rule-style="solid" color="#AAAAAA" leader-length="150mm"/>
											</fo:inline>
										</fo:block>-->
										...
										住所:　　
											<fo:inline padding-left="6mm">
												<!-- 都道府県と住所を正規化して取得 --><!--ここから下は、ポーターズでの住所詳細(個人)に都道府県が入っている人といない人で統一されていあにため、都道府県が重複しないためのコード-->
												<xsl:variable name="pref" select="normalize-space(Sales.P_Candidate/Candidate/Person.U_6E906CC3D68B8B4F24A2BA8C5398E7)"/>
												<xsl:variable name="addr" select="normalize-space(Sales.P_Candidate/Candidate/Person.P_Street)"/>

												<xsl:choose>
												<!-- pref が空でない AND addr の先頭が pref と一致する場合 -->
												<xsl:when test="string-length($pref) &gt; 0 and substring($addr, 1, string-length($pref)) = $pref">
													<!-- pref を1回表示し、addr の先頭の pref 部分を切り落として続ける -->
						...
						<fo:table table-layout="fixed" width="100%" margin-top="10mm">
							<fo:table-body>
